...
//    if (stop==1)
 //       return 0;
    ...
    for (i = 0; i < ladrip_number && i < total; i++) {
     ...
     //       continue;
	 	c = ((ladrip_mac[i][2] << 4) | ladrip_mac[i][3]);
        hb[1] = c;
		c = ((ladrip_mac[i][4] << 4) | ladrip_mac[i][5]);
		hb[2] = c;

        if (i == 0)
            fprintf(stdout, "\n------------- send heartbeat data ----------------\n");
        iop_send_data(hb, sizeof(hb));
    }


    tv.tv_sec =inter;   //初始时间2s
    tv.tv_usec = 0;
...
int ladrip_send_e2(struct rg_thread *thread)
{
 ...
 //       return 0;
    ...
    for (i = 0; i < ladrip_number&& i < total; i++){
        ...
        iop_send_data(e2, sizeof(e2));// 发送1个有用的E2
        iop_send_data(e2_2, sizeof(e2_2));
...
static int iop_send_data_func_kyee(uint8_t *buf, int buf_len)
{
    fprintf(stdout, "\n-------------are you kidding me?----------------\n");
...
int  main(int argc, char **argv)
{
    ...
    if (argc  != 9) {
        printf("param err (acgc != 8)\n");
        return -1;
    }

#if 1
    vscp_rfid_register_write_func(iop_send_data_func_kyee);
...
//    context->serverAddr = strdup("iotc.ruijie.com.cn");
    ...
    for (i = 0; i < context->ins_num; i++) {
        context->ins_array[i].rfid_type = RFID_SEN_T_MWF; /* TODO */
        context->ins_array[i].model = RFID_SEN_M_NRF905;
...
//        (void *)client_global, 10);
    ...
    if (strcmp(start_data, "1") == 0) {
        fprintf(stdout, "\n------------- start data ----------------\n");
        rg_thread_add_timer(context->vscp_master, kyee_send_hb, (void *)context, inter);
    } else if (strcmp(start_data, "2") == 0) {
    	fprintf(stdout, "\n------------- start data ----------------\n");
		rg_thread_add_timer(context->vscp_master, kyee_send_off, (void *)context, inter);
	} else if (strcmp(start_data, "3") == 0) {
    	fprintf(stdout, "\n------------- start data ----------------\n");
		rg_thread_add_timer(context->vscp_master, kyee_send_out, (void *)context, inter);
	} else {
        fprintf(stdout, "\n------------- stop data ----------------\n");
    }
