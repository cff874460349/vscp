                                 ...
                                 size_t bufferLen)
{
    ...
    case LWM2M_TYPE_OBJECT_INSTANCE:
    {
        ...
        res = utils_intToText(tlvP->id, (uint8_t *)uriStr + uriLen, URI_MAX_STRING_LEN - uriLen);
        if (res <= 0) return -1;
        uriLen += res;
        ...
        {
            res = prv_serializeLinkData(contextP, tlvP->value.asChildren.array + index, &uri, (uint8_t *)uriStr, uriLen, buffer + head, bufferLen - head);
            if (res < 0) return -1;
            head += res;
        }
    }
                      ...
                      uint8_t ** bufferP)
{
    ...
    baseUriLen = uri_toString(uriP, (uint8_t *)baseUriStr, URI_MAX_STRING_LEN, NULL);
    ...
    for (index = 0; index < size && head < PRV_LINK_BUFFER_SIZE; index++)
    {
        ...
        res = prv_serializeLinkData(contextP, dataP + index, uriP, (uint8_t *)baseUriStr, baseUriLen, bufferLink + head, PRV_LINK_BUFFER_SIZE - head);
        if (res < 0) return -1;
        head += res;
    }
