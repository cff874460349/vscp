...
#endif
    static unsigned long msg_num = 0;
	if (info_buf.used + len >= 4096) {
		ret = vscp_iot_update_data(client_global, sensor_mac, info_buf.buf, msg_num);
		if (ret) {
		   VSCP_DBG_ERR("rg_thread_read_rssi() CALL rssi_update_data() failed!\n");
		}
		memset(&info_buf, 0, sizeof(info_buf));
	}
	tag_msg_info_t *p = (tag_msg_info_t *)info_buf.buf+info_buf.used;
	p->msg_len = len;
	memcpy(p->msg, data, p->msg_len);
	msg_num++;
	info_buf.used += p->msg_len;
	if ((info_buf.used >= 4096) || ((msg_num%10)==0)) {
		ret = vscp_iot_update_data(client_global, sensor_mac, info_buf.buf, msg_num);
		if (ret) {
		   VSCP_DBG_ERR("rg_thread_read_rssi() CALL rssi_update_data() failed!\n");
		}
...
//    fprintf(stdout, "\n\n\n\n\n\n\n\n\n\n\n\n-------------send ewell sts ----------------\n");
    ...
    iop_send_data(config_ewell.sensor_mac, sts,sizeof(sts));
...
//    fprintf(stdout, "\n\n\n\n\n\n\n\n\n\n\n\n-------------send ewell dat ----------------\n");
    ...
    for (i = 0; i < MAX_TAG_NUM && i < config_ewell.tag_num; i++){
        ...
        //memcpy(dat+16, buf, 8);
        iop_send_data(config_ewell.sensor_mac, dat,sizeof(dat));
    }
	tv.tv_sec = 0;
...
//    context->serverAddr = strdup("iotc.ruijie.com.cn");
	...
	sprintf(config_ewell.id, "%04d", rand()%10000);
...
//        (void *)client_global, 10);
	...
	if (config_ewell.enable) {
	    ...
	    if (config_ewell.ewell_data_dat.enable) {
	        fprintf(stdout, "\n------------- start ewell dat ----------------\n");
	        rg_thread_add_timer(context->vscp_master, ewell_send_dat, (void *)context, r);
	    } else {
	        fprintf(stdout, "\n------------- stop ewell dat ----------------\n");
	    }
	}
