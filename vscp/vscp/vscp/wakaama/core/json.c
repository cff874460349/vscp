                      ...
                      size_t bufferLen)
{
    ...
    case LWM2M_TYPE_MULTIPLE_RESOURCE:
    {
        ...
        {
            res = prv_serializeData(tlvP->value.asChildren.array + index, (uint8_t *)uriStr, uriLen, buffer + head, bufferLen - head);
            if (res < 0) return -1;
            head += res;
        }
    }
                      ...
                      uint8_t ** bufferP)
{
    ...
    baseUriLen = uri_toString(uriP, (uint8_t *)baseUriStr, URI_MAX_STRING_LEN, &rootLevel);
         ...
         || targetP->type == LWM2M_TYPE_MULTIPLE_RESOURCE))
    {
        ...
        res = utils_intToText(targetP->id, (uint8_t *)baseUriStr + baseUriLen, URI_MAX_STRING_LEN - baseUriLen);
        if (res <= 0) return 0;
        baseUriLen += res;
